...
- **Мгновенные уведомления** — параметр `notify: instant` для любого датчика: уведомление при каждом изменении значения (bool, числовое, строковое), первое получение игнорируется
